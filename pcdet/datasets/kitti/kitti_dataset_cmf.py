...
class KittiDatasetCMF(DatasetTemplate):
    ...
    def get_lidar_pseudo(self, idx):
        ...
        # num_ponit_features_pseudo = len(self.dataset_cfg.POINT_FEATURE_ENCODING.src_feature_list_mm)
        point_pseudo = np.fromfile(str(lidar_pseudo_file), dtype=np.float32).reshape(-1, 9)  
        # print(point_pseudo[:2,:])  
    ...
    def evaluation_by_distance(self, det_annos, class_names, distance_interval = [0,10,40,200], **kwargs):
        if 'annos' not in self.kitti_infos[0].keys():
            return None, {}
        ...
        ap_dict_info = {}
        for i in range(3):
            loc_min , loc_max = distance_interval[i], distance_interval[i+1]
            single_det_annos , single_gt_annos = [], []
            for det_anno, gt_anno in zip(eval_det_annos, eval_gt_annos):
                cur_det_anno = det_anno.copy()
                location_norm =  np.linalg.norm(cur_det_anno['location'], axis=1)
                dis_mask = (location_norm > loc_min) & (location_norm < loc_max)
                for key in cur_det_anno.keys():
                    if key != 'frame_id':
                        cur_det_anno[key] = cur_det_anno[key][dis_mask]
                single_det_annos.append(cur_det_anno)

                cur_gt_anno = gt_anno.copy()
                location_norm =  np.linalg.norm(cur_gt_anno['location'], axis=1)
                care_boolean = cur_gt_anno['name'] != 'DontCare'
                dis_mask = (location_norm > loc_min) & (location_norm < loc_max)
                for key in cur_gt_anno.keys():
                    if key != 'gt_boxes_lidar':
                        cur_gt_anno[key] = cur_gt_anno[key][dis_mask]
                    else:
                        cur_gt_anno[key] = cur_gt_anno[key][dis_mask[care_boolean]]
                single_gt_annos.append(cur_gt_anno)
            print('len of gt %d, len of det %d'%(len(single_gt_annos), len(single_det_annos)))
            
            ap_result_str, ap_dict = kitti_eval.get_official_eval_result(single_gt_annos, single_det_annos, class_names)
            ap_result_str = f'Result between {distance_interval[i]} and {distance_interval[i+1]} distance is:' + ap_result_str + '\n'
            ap_result_str_list.append(ap_result_str)
            for key, val in ap_dict.items():
                ap_dict_info[key + 'Dis%d'%i] = val

        return ap_result_str_list, ap_dict_info
    ...
    def __getitem__(self, index):
        ...
        points = self.get_lidar(sample_idx) # velodyne
        points_pseudo = self.get_lidar_pseudo(sample_idx) # velodyne_depth

        points = np.concatenate([points[:,:3], np.zeros((points.shape[0],6)), np.ones((points.shape[0],1))*2],axis=-1)
        points_pseudo = np.concatenate([points_pseudo, np.ones((points_pseudo.shape[0],1))*1],axis=-1)
        points = np.concatenate([points, points_pseudo],axis=0)

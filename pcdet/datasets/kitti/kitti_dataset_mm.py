...
class KittiDatasetMM(DatasetTemplate):
    ...
    def create_groundtruth_database(
        ...
        database_save_path = Path(self.root_path) / (
            "gt_database_mm" if split == "train" else ("gt_database_%s_mm" % split)
        )
    ...
    def __getitem__(self, index):
        ...
        if "annos" in info:
            ...
            if (self.dataset_cfg.get("USE_VAN", None) is True) and (
                self.training is True
            ):
                gt_names = np.array(
                    [
                        "Car" if gt_names[i] == "Van" else gt_names[i]

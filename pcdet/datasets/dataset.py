...
class DatasetTemplate(torch_data.Dataset):
    ...
    def prepare_data(self, data_dict):
        ...
        Returns:
            data_dict:
                ...
                ...
        """
        # data_dict['raw_points'] = points = data_dict['points'][data_dict['points'][:, -1] == 2]
        # data_dict['raw_gt_boxes'] = data_dict['gt_boxes']
        # common_utils.save_point_cloud_to_ply(data_dict['points'][:,:3], 'pts.ply')
        # common_utils.save_gt_boxes_to_ply(data_dict['gt_boxes'], 'boxes.ply')
        if self.training:
            ...
            if 'mm' in data_dict:
                if self.dataset_cfg.get('LATER_FUSION', True):
                    ...
                    if self.training:
                        ...
                        index = np.arange(points_mm.shape[0])
                        np.random.shuffle(index)
                        points_mm2 = points_mm[index[int(points_mm.shape[0]*self.input_discard_rate):]]
                    else:
    ...
    def collate_batch(self, batch_list, _unused=False):
        ...
        point_key_dict=['raw_points', 'points', 'voxel_coords', 'points_mm', 'voxel_coords_mm']
        for i in range(1, 10):
        ...
        boxes_key = ['gt_boxes','raw_gt_boxes']
        for i in range(1, 10):

...
class WaymoDatasetMM(DatasetTemplate):
    ...
    def get_lidar_mm(self, sequence_name, sample_idx):
        lidar_file = self.data_path / sequence_name / ('%04d.npy' % sample_idx)
        point_features = np.load(lidar_file)  # (N, 7): [x, y, z, intensity, elongation, NLZ_flag]
        ...
        if self.dataset_cfg.get('POINTS_TANH_DIM', None) is None:
            ...
            for dim_idx in self.dataset_cfg.POINTS_TANH_DIM:
                points_all[:, dim_idx] = np.tanh(points_all[:, dim_idx])
        point_part1 = np.concatenate((points_all, np.ones((points_all.shape[0],1))*2),axis=-1)
        point_part2 = np.concatenate((points_all, np.ones((points_all.shape[0],1))),axis=-1)
        return np.concatenate((point_part1,point_part2),axis=0)
    ...
    def __getitem__(self, index):
        ...
        if self.use_shared_memory and index < self.shared_memory_file_limit: # false
            sa_key = f'{sequence_name}___{sample_idx}'
            points = SharedArray.attach(f"shm://{sa_key}").copy()
        else:
            points = self.get_lidar_mm(sequence_name, sample_idx)

        if self.dataset_cfg.get('SEQUENCE_CONFIG', None) is not None and self.dataset_cfg.SEQUENCE_CONFIG.ENABLED: # false
        ...
        input_dict.update({"double_box_lidar": True})

        if 'annos' in info: # true
            annos = info['annos']
            annos = common_utils.drop_info_with_name(annos, name='unknown')

            if self.dataset_cfg.get('INFO_WITH_FAKELIDAR', False): # false
                gt_boxes_lidar = box_utils.boxes3d_kitti_fakelidar_to_lidar(annos['gt_boxes_lidar'])
            else:
                gt_boxes_lidar = annos['gt_boxes_lidar']

            if self.dataset_cfg.get('TRAIN_WITH_SPEED', False): # false
                assert gt_boxes_lidar.shape[-1] == 9
            else:
                gt_boxes_lidar = gt_boxes_lidar[:, 0:7]

            if self.training and self.dataset_cfg.get('FILTER_EMPTY_BOXES_FOR_TRAIN', False): # true

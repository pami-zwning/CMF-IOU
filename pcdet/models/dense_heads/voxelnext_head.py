...
class VoxelNeXtHead(nn.Module):
    ...
    def get_loss(self):
        ...
        for idx, pred_dict in enumerate(pred_dicts):
            ...
            # batch_zero = batch_indices == 0
            # spatial_indices_batch = spatial_indices[batch_zero]
            # pred_heatmap_batch = pred_dict['hm'][batch_zero]
            # target_heatmap_batch = target_dicts['heatmaps'][idx][batch_zero]
            # pred_img = pred_heatmap_batch.new_zeros((188, 188))
            # pred_img[spatial_indices_batch[:, 1].long(), spatial_indices_batch[:, 0].long()] = pred_heatmap_batch[:,0]
            # target_img = pred_heatmap_batch.new_zeros((188, 188))
            # target_img[spatial_indices_batch[:, 1].long(), spatial_indices_batch[:, 0].long()] = target_heatmap_batch[:,0]
            # plt.figure(figsize=(10, 5))

            # pred_img = pred_img.detach().cpu().numpy()
            # target_img = target_img.detach().cpu().numpy()
            # plt.subplot(1, 2, 1)
            # sns.heatmap(pred_img, cmap='cividis', vmin=0, vmax=1, cbar=True)
            # plt.title('Prediction Heatmap')

            # plt.subplot(1, 2, 2)
            # sns.heatmap(target_img, cmap='cividis', vmin=0, vmax=1, cbar=True)
            # plt.title('Target Heatmap')
            # plt.tight_layout()
            # plt.savefig('./image/heatmaps_error.png', dpi=300, bbox_inches='tight')
            # # end heatmap visualization
...
class VoxelNeXtHeadTED(nn.Module):
    def __init__(self, model_cfg, input_channels, num_class, class_names, grid_size, point_cloud_range, voxel_size,
                 predict_boxes_when_training=False):
        super().__init__()
        self.model_cfg = model_cfg
    ...
    def forward(self, data_dict):
        ...
        for i in range(rot_num):
            ...
            x = data_dict['encoded_spconv_tensor' + rot_num_id]

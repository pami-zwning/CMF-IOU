...
class ResidualCoderNuscenes(object):
    def __init__(self, code_size=10, **kwargs):
        super().__init__()
        self.code_size = code_size

    def encode_torch(box_encodings, anchors):
        """
        Args:
            box_encodings:  (B, N, 7 + ?) x, y, z, w, l, h, r, custom values
            anchors: (B, N, 7 + C) or (N, 7 + C) [x, y, z, dx, dy, dz, heading, ...]
        ...
        xa, ya, za, dxa, dya, dza, ra, cas, vxa, vya = torch.split(anchors, 1, dim=-1)
        xg, yg, zg, dxg, dyg, dzg, rg, cgs, vxg, vyg = torch.split(boxes, 1, dim=-1)
        ...
        rts = rg - ra
        cts = cgs - cas
        dvx = torch.log(vxg / vxa)
        dvy = torch.log(vyg / vya)

        return torch.cat([xt, yt, zt, dxt, dyt, dzt, rts, cts, dvx, dvy], dim=-1)
